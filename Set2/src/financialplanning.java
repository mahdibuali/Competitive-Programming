...
public class financialplanning {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int cost = in.nextInt();
            int days = cost / profit;
            if (l.containsKey(days)) {
                int[] x = new int[]{profit + l.get(days)[0], cost + l.get(days)[1]};
                l.replace(days, x);
            }
            else {
                l.put(days, new int[]{profit, cost});
            }
        }
        int rev = 0;
        int prf = 0;
        ...
        while (true) {
            if (l.containsKey(i)) {
                rev += prf + (i + 1) * l.get(i)[0] - l.get(i)[1];
                prf += l.get(i)[0];
            }
            else {
                rev += prf;
